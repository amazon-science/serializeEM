...
from akg_llmer.utils import get_labels, serial_entity
...
class DittoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if isinstance(path, list):
            ...
            if file_type in ['csv', 'pkl']:
                if file_type == 'csv':
                    df = pd.read_csv(path)
                    self.labels = df.label.values
                else:
                    num_neg = config_data['num_neg']
                    idx_pos, idx_neg = get_labels(path=path)
                    df = pd.DataFrame(np.stack([np.repeat(
                        idx_pos, num_neg+1), idx_neg[:, :num_neg+1].flatten()]).T, columns=['ltable_id', 'rtable_id'])
                    self.labels = np.zeros(len(df))

                seq_l, seq_r = serial_entity(
                    config_data, stype=config_data['stype'], indices=df)
                self.pairs = list(zip(seq_l, seq_r))
            elif file_type == 'txt':
                lines = open(path)
                for line in lines:
